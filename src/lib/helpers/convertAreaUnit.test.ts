...
describe('convertAreaUnit', () => {
	...
	describe('ForceUnit - Metric units', () => {
		it('should force conversion to m² when forceUnit is m2', () => {
			expect(convertAreaUnit(20000, 'metric', 'm2')).toEqual({ area: 20000, unit: 'm²' });
		});

		it('should force conversion to a when forceUnit is a', () => {
			expect(convertAreaUnit(20000, 'metric', 'a')).toEqual({ area: 200, unit: 'a' });
		});

		it('should force conversion to ha when forceUnit is ha', () => {
			expect(convertAreaUnit(5000, 'metric', 'ha')).toEqual({ area: 0.5, unit: 'ha' });
		});

		it('should force conversion to km² when forceUnit is km2', () => {
			expect(convertAreaUnit(500000, 'metric', 'km2')).toEqual({ area: 0.5, unit: 'km²' });
		});
	});

	describe('ForceUnit - Imperial units', () => {
		it('should force conversion to ft² when forceUnit is ft2', () => {
			expect(convertAreaUnit(1, 'imperial', 'ft2')).toEqual({
				area: 10.763910416709722,
				unit: 'ft²'
			});
		});

		it('should force conversion to yd² when forceUnit is yd2', () => {
			expect(convertAreaUnit(10, 'imperial', 'yd2')).toEqual({
				area: 11.959900463010802,
				unit: 'yd²'
			});
		});

		it('should force conversion to acre when forceUnit is acre', () => {
			expect(convertAreaUnit(8093.712, 'imperial', 'acre')).toEqual({
				area: 2,
				unit: 'acre'
			});
		});
		...
		it('should force conversion to mi² when forceUnit is mi2', () => {
			expect(convertAreaUnit(2589988.11, 'imperial', 'mi2')).toEqual({
				area: 1,
				unit: 'mi²'
			});
		});
	...
	describe('ForceUnit - Invalid unit type fallback to auto', () => {
		it('should fallback to auto when metric is selected but imperial unit is forced', () => {
			...
			expect(convertAreaUnit(20000, 'metric', 'acre')).toEqual({ area: 2, unit: 'ha' });
		});

		it('should fallback to auto when imperial is selected but metric unit is forced', () => {
			// Should auto-select 'yd²' instead of forcing 'ha'
			expect(convertAreaUnit(1, 'imperial', 'ha')).toEqual({
		...
		it('should fallback to auto when metric is selected but another invalid imperial unit is forced', () => {
			...
			expect(convertAreaUnit(5000000, 'metric', 'mi2')).toEqual({ area: 5, unit: 'km²' });
		});
	});
