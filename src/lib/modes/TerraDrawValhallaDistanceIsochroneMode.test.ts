...
describe('TerraDrawValhallaDistanceIsochroneMode', () => {
	afterEach(() => {
		vi.clearAllMocks();
	});
	...
	describe('lifecycle', () => {
		it('initialises with correct mode name', () => {
			const mode = new TerraDrawValhallaDistanceIsochroneMode({ url: 'https://test.com' });
			expect(mode.mode).toBe('distance-isochrone');
		});
		...
		it('defaults costingModel to auto', () => {
			const mode = new TerraDrawValhallaDistanceIsochroneMode({ url: '' });
		...
		it('handles API error gracefully', async () => {
			...
			await vi.waitFor(() => {
				expect(consoleSpy).toHaveBeenCalledWith(
					'Valhalla distance isochrone error:',
					expect.any(Error)
				);
			});
