...
export type DistanceIsochronePointStyling = {
	pointColor?: HexColor;
	pointWidth?: number;
...
export type ValhallaDistanceIsochroneModeOptions = {
	url: string;
	costingModel?: costingModelType;
	contours?: Contour[];
	styles?: Partial<DistanceIsochronePointStyling>;
};
...
export class TerraDrawValhallaDistanceIsochroneMode extends TerraDrawBaseDrawMode<DistanceIsochronePointStyling> {
	...
	constructor(options: ValhallaDistanceIsochroneModeOptions) {
	...
	private async computeIsochrone(
		...
		try {
			...
			console.error('Valhalla distance isochrone error:', error);
		}
	}
}
